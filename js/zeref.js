...
const spriteBase = "assets/zeref_sprites/";
const tracks = {
  natsu: {
    title: "Natsu Theme",
...
function drawFrame() {
  ...
  stateLabel.textContent = state.mode === "walk" ? "Caminando" : state.mode === "magic" ? "Oscura" : state.mode === "attack" ? "Ataque" : state.mode === "damage" ? "Danado" : state.mode === "dead" ? "Caido" : "Reposo";
...
function burstFlame() {
  for (let i = 0; i < 10; i += 1) {
    ...
    ember.style.cssText = `position:absolute;left:${state.x + 4}%;top:${state.y - 16}%;z-index:7;width:${8 + Math.random() * 14}px;aspect-ratio:1;border-radius:50%;background:#7c35ff;box-shadow:0 0 18px #d9a0ff;pointer-events:none;`;
